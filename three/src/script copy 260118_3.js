...
let y = 0; // current rotation value
let ySpeed = 0.15; // speed of slow rotation in idle state

//zerlegung
let disassemblyProgress = 0; // 0 = assembled, 100 = fully disassembled
let animationState = 'inRuhe'; // 'inRuhe', 'rotating', 'disassembling', or 'assembling'
const disassemblySpeed = 1; // speed at which parts disassemble
const animationViewRotation = Math.PI / 0.61; // target rotation when animation starts (45°)
...
function setup() {
  ...
  controls.minPolarAngle = Math.PI / 2.5; // 60 degrees (can look from above)
  controls.maxPolarAngle = Math.PI / 2.5; // 90 degrees (horizontal limit)
...
function updateDisassembly(deltaTime) {
  ...
  if (animationState == 'rotating') {
    ...
    if (Math.abs(rotationDifference) < 0.01) {
      ...
      animationState = disassemblyProgress == 0 ? 'disassembling' : 'assembling';
    } else {
  ...
  if (knob_left && knob_left.userData.originalPosition) {
    ...
    knob_left.position.y =
      knob_left.userData.originalPosition.y + progress1 * 0.008 + progress2 * 0.0013;
  }
  ...
  if (knob_right && knob_right.userData.originalPosition) {
    ...
    knob_right.position.y =
      knob_right.userData.originalPosition.y + progress1 * 0.008 + progress2 * 0.0013;
  }
