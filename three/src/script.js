...
let frame_counter;
let gear;
let parts = []; // global parts array for click detection

//automatische drehung
let y = 0;
let ySpeed = 0.2; //Drehgeschwindigkeit
...
function initCamera() {
  ...
  camera.position.y = -1.5; // a bit from above
  camera.position.z = 2;
...
function initLight() {
  ...
  pointLight = new THREE.PointLight("rgb(255, 255, 255)", 35); //links
  pointLight.position.x = -2;
  pointLight.position.y = 1;
  pointLight.position.z = 5;
  scene.add(pointLight);

  pointLight = new THREE.PointLight("rgb(255, 255, 255)", 5); //links vorne
  pointLight.position.x = 2;
  pointLight.position.y = 1;
  pointLight.position.z = 5;
  scene.add(pointLight);

  pointLight = new THREE.PointLight("rgb(255, 255, 255)", 35); //oben rechts
  pointLight.position.x = 2;
  pointLight.position.y = 10;
  pointLight.position.z = -6;
  scene.add(pointLight);

  pointLight = new THREE.PointLight("rgb(255, 255, 255)", 35); //rechts
...
function loadObjects() {
  ...
  gltfLoader.load(
    "./three/static/models/pinhole/weitwinkelkamera_ganz_repariert_3.glb",
    objectLoaded,
  );
}
...
function updateDisassembly(deltaTime) {
  if (animationState == "rotating") {
    ...
    // downscaling
    let currentScale = pinhole.scale.x;
    if (currentScale > targetScale + 0.5) {
      let newScale =
        currentScale +
        (targetScale - currentScale) * Math.min(1, deltaTime * 6);
      pinhole.scale.set(newScale, newScale, newScale);
    } else {
    ...
    //then: scaling
      ...
      if (currentScale < initialScale - 0.5) {
        let newScale =
          currentScale +
          (initialScale - currentScale) * Math.min(1, deltaTime * 6);
        pinhole.scale.set(newScale, newScale, newScale);
      } else {
        animationState = "inRuhe";
        y = pinhole.rotation.y;
...
function onClick(event) {
  ...
  const intersects = raycaster.intersectObject(pinhole, true);

  if (intersects.length > 0 && animationState == "inRuhe") {
    animationState = "rotating";
  }
}
